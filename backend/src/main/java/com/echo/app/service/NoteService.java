...
public class NoteService {
    ...
    public PagedResponse<NoteResponse> getNotes(
        ...
        Pageable pageable = PageRequest.of(page, clampedSize, Sort.by(direction, sortField));

        Page<Note> notePage = noteRepository.findAll(pageable);
        List<NoteResponse> items = notePage.getContent().stream().map(NoteResponse::fromEntity).toList();
     ...
     */
    public NoteResponse createNote(CreateNoteRequest request, String clientIp, String userAgent) {
        ...
        String sanitizedAuthor = request.getAuthor() != null ? sanitize(request.getAuthor().trim()) : null;
